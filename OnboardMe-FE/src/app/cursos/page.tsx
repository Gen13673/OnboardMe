...
function CourseCard({
  ...
  return (
    <div className="relative bg-white/80 backdrop-blur rounded-xl shadow-md hover:shadow-lg transition-all duration-200 border-0 overflow-visible">
      {/* Header */}
...
export default function CursosPage() {
  ...
  useEffect(() => {
    ...
    if (!userId) return;
    fetchCourses(userId);
  }, [user?.id]); // Dependemos solo del ID, que es un valor estable
  ...
  }, [courses, user?.id]); // Dependemos del ID y de `courses`
  ...
  const handleMarkFavorite = async (courseId: number) => {
    try {
      ...
      if (wasFavorite) toast.info("Curso quitado de favoritos.", { containerId: "app" });
      else toast.success("Curso marcado como favorito.", { containerId: "app" });
    } catch (error) {
      toast.error("Error al marcar/desmarcar como favorito.", { containerId: "app" });
      console.error(error);
    }
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-600">
                  Gestiona y accede a tus cursos de formación
                </p>
              </div>
            </div>
          </div>
