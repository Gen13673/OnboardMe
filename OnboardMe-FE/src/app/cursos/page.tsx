...
import { ExternalLink } from "lucide-react" // Cambiado a ExternalLink
...
export default function CursosPage() {
  ...
  const handleMarkFavorite = async (courseId: number) => {
    try {
      ...
      if (wasFavorite) {
        toast.info("Curso quitado de favoritos.")
      } else {
        toast.success("Curso marcado como favorito.")
      }
    } catch (error) {
  ...
  const filteredAndSortedCourses = courses
    .filter((course) => {
      ...
      if (searchTerm) {
        ...
        const matchesInstructor = `${course.createdBy.firstName} ${course.createdBy.lastName}`
          .toLowerCase()
    ...
    .sort((a, b) => {
      switch (sortBy) {
        ...
        case "progress-asc":
          return (progressMap[a.id] || 0) - (progressMap[b.id] || 0)
        default:
          return 0
      }
    })
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              {/* Icono de cursos - color ajustado */}
          ...
          {currentCourses.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {currentCourses.map((course) => (
                <div
                  key={course.id}
                  className="bg-white/80 backdrop-blur rounded-xl shadow-md hover:shadow-lg transition-all duration-200 border-0 overflow-hidden"
                >
                  {/* Card Header */}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() => handleMarkFavorite(course.id)}
                            className="p-1 h-8 w-8 rounded hover:bg-gray-100 transition-colors"
                          >
                            <svg
                              className={`h-4 w-4 transition-colors ${isFavorite(course)
                                  ? "fill-yellow-400 text-yellow-400"
                                  : "text-gray-400 hover:text-yellow-400"
                                }`}
                              fill={isFavorite(course) ? "currentColor" : "none"}
                              stroke="currentColor"
                              viewBox="0 0 24 24"
                            >
                              <path
                    ...
                    {course.sections && course.sections.length > 0 && (
                      <button
                        onClick={() => {
                          ...
                          if (progress === 100 && totalSections > 0) {
                            // Si está 100% completo, enlazar a la última sección
                            sectionIdToUse = course.sections[totalSections - 1].id
                          } else if (completedSectionsCount < totalSections) {
                            // Enlazar a la siguiente sección incompleta
                            sectionIdToUse = course.sections[completedSectionsCount].id
                          } else {
                            // Fallback: si no hay progreso o secciones, enlazar a la primera
                            sectionIdToUse = course.sections[0].id
                          }

                          if (sectionIdToUse) {
                            window.open(
                              `/cursos/${course.id}?seccion=${sectionIdToUse}`,
                              "_blank",
                              "noopener,noreferrer",
                            )
                          }
                        ...
                        title={
                          (progressMap[course.id] || 0) === 100
                            ? "Revisar curso"
                            : (progressMap[course.id] || 0) === 0
                              ? "Empezar curso"
                              : "Continuar curso"
                        }
                      >
                        <ExternalLink
                          className="w-5 h-5 text-blue-600 transition-colors duration-150 hover:text-blue-700"
                        />
                      </button>
                    )}
                  </div>
                  ...
                  <div className="px-6 pb-6 space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="flex items-center gap-3">
                        {/* Avatar de instructor - color ajustado */}
                      ...
                      <div className="flex items-center gap-2">
                        <svg className="h-4 w-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <path
                            ...
                            strokeLinejoin="round"
                            strokeWidth={2}
                            d="M2 13h20v6a2 2 0 01-2 2H4a2 2 0 01-2-2v-6z"
                          />
                        </svg>
                        <div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className="w-full">
                          <div className="text-xs text-gray-600 mb-1">Progreso: {progressMap[course.id] || 0}%</div>
                      ...
                      {course.sections && course.sections.length > 0 && (
                        <button
                          ...
                          className={`inline-flex items-center gap-2 px-3 py-1.5 text-sm font-medium rounded-lg transition-all duration-200 ${expandedCourses.has(course.id)
                              ? "text-gray-600 hover:text-gray-800 hover:bg-gray-50 border border-gray-200 hover:border-gray-300"
                              : "text-gray-600 hover:text-gray-800 hover:bg-gray-50 border border-gray-200 hover:border-gray-300"
                            }`}
                        >
                          <svg
                            className={`h-3 w-3 transition-transform duration-200 ${expandedCourses.has(course.id) ? "rotate-180" : ""
                              }`}
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          ...
                          <span>{expandedCourses.has(course.id) ? "Ocultar" : "Ver"} Secciones</span>
                        </button>
                      )}
                    </div>
                                  ...
                                  <div className="flex items-start justify-between gap-3">
                                    <div className="flex-1 min-w-0">
                                      <div className="flex items-center gap-2 mb-2">
                                        <span
                                          className={`text-xs font-semibold rounded-full w-6 h-6 flex items-center justify-center ${course.currentSection &&
                                              course.sections[index].id === course.currentSection.id
                                              ? "bg-blue-500 text-white" // Sección actual: azul fuerte, texto blanco
                                              : index + 1 <= (progressMap[course.id] / 100) * course.sections.length
                                                ? "bg-blue-400 text-gray-700" // Sección completada: azul más claro, texto gris
                                                : "bg-gray-300 text-gray-700" // Sección incompleta: gris, texto gris
                                            }`}
                                        >
                                          {index + 1}
                                        </span>
                                        <div className="flex items-center justify-between">
                                          ...
                                          <button
                                            onClick={() =>
                                              window.open(
                                                `/cursos/${course.id}?seccion=${section.id}`,
                                                "_blank",
                                                "noopener,noreferrer",
                                              )
                                            }
                                      ...
                                      {section.contentURL && (
                                        <div className="flex items-center gap-2 mt-2">
                                          <button
                                            onClick={() =>
                                              window.open(section.contentURL, "_blank", "noopener,noreferrer")
                                            }
                                          ...
                                          <button
                                            onClick={() => {
                                              navigator.clipboard.writeText(section.contentURL)
                                            }}
              ...
              <div className="flex gap-1">
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                  <button
                    ...
                    className={`w-10 h-10 text-sm rounded-md transition-colors ${currentPage === page ? "bg-blue-500 text-white" : "border border-gray-300 hover:bg-gray-50"
                      }`}
                  >
                    {page}
                  </button>
