...
export default function CoursePage() {
  ...
  const [showExamWarning, setShowExamWarning] = useState(false);
    ...
    // Cada vez que cambio de sección, hago scroll al comienzo del contenedor
    useEffect(() => {
    ...
    if (course) {
      ...
      if (sectionIdParam && course.sections) {
        const index = course.sections.findIndex(
          (sec) => sec.id === Number(sectionIdParam),
        );
        if (index !== -1) {
  ...
  useEffect(() => {
    ...
    getCourseById(courseId).then((data) => {
      const sortedSections = data.sections?.sort((a, b) =>
        a.order && b.order ? Number(a.order) - Number(b.order) : 0,
      );
  ...
  useEffect(() => {
    ...
    // resets comunes al cambiar de sección
    setExamResult(null);
    setAnswers({});

    // ===== EXAM =====
    if (section.content?.type === "EXAM") {
      // inicializar answers vacíos por pregunta
      const init: Record<number, Set<number>> = {};
      section.content.questions?.forEach((q) => {
        init[q.id] = new Set();
      });
      setAnswers(init);

      // si ya hay resultado, aplicar y limpiar storage
      ...
      // recuperar resultado del backend
      if (user?.id) {
        getExamResult(section.id, user.id)
          ...
          .catch(() => {
            ...
            // restaurar respuestas desde localStorage si nunca se envió
            try {
              const raw = localStorage.getItem(
                answersKey(user.id!, section.id),
              );
              if (raw) {
    ...
    // ===== SURVEY =====
    if (section.content?.type === "SURVEY") {
      ...
      // intentar recuperar respuesta previa para bloquear edición
      if (user?.id) {
        getSurveyResult(section.id, user.id)
          .then((res) => {
            if (!res) return;
            setSurveyResult(res);
            const map: Record<number, Set<number>> = {};
            res.results.forEach((r) => {
              map[r.questionId] = new Set(r.selectedOptionIds);
            });
            setAnswers(map);
          })
          .catch(() => {});
  ...
  useEffect(() => {
    const section = course?.sections?.[currentSectionIndex];
    if (!section || section.content?.type !== "EXAM" || !user?.id || examResult)
      return;
  ...
  const timerKey = (userId: number, sectionId: number) =>
    `examTimer:${userId}:${sectionId}`;
  const answersKey = (userId: number, sectionId: number) =>
    `examAnswers:${userId}:${sectionId}`;
  ...
  const handleSubmitSurvey = async () => {
    ...
    try {
      const payload = {
        answers: Object.entries(answers).map(([qid, set]) => ({
          questionId: Number(qid),
          selectedOptionIds: Array.from(set ?? []),
        })),
      };

      await submitSurvey(section.id, user.id, payload);

      const res = await getSurveyResult(section.id, user.id);
      if (res) {
        setSurveyResult(res);
  ...
  const toggleAnswerSingleAware = (
    ...
    setAnswers((prev) => {
      const next = { ...prev };
      if (isSingle) {
        next[questionId] = new Set([optionId]); // forzamos 1 sola
      } else {
        const set = new Set(next[questionId] ?? []);
        set.has(optionId) ? set.delete(optionId) : set.add(optionId);
        next[questionId] = set;
  ...
  const isLastSection = currentSectionIndex === totalSections - 1;
    ...
    if (!isNaN(courseId) && user?.id && currentSection?.id) {
      ...
      if (!updatedSectionIds.has(currentSection.id)) {
        try {
          ...
          localStorage.setItem(
            storageKey,
            JSON.stringify([...updatedSectionIds]),
          );
        ...
        console.log(
          "⏩ Sección ya actualizada anteriormente:",
          currentSection.id,
        );
      }
    }
    ...
    if (isLastSection) {
      ...
      const finishedSet = new Set<number>(
        finishedRaw ? JSON.parse(finishedRaw) : [],
      );
    ...
    const handleNextSection = async () => {
        // Si la próxima sección es un EXAM, chequeamos si ya está resuelto
        if (nextSectionIsExam && user?.id && nextSection) {
            try {
                const res = await getExamResult(nextSection.id, user.id);

                // Si tiene resultados, ya se rindió → avanzamos sin mostrar modal
                if (res && res.results && res.results.length > 0) {
                    await goToNextSection();
                    return;
                }
            } catch (e) {
                console.warn("No hay resultado previo para el examen siguiente:", e);
            }

            // No hay resultado → mostramos cartel de advertencia
            setShowExamWarning(true);
            return;
        }

        // Si la próxima sección no es EXAM, navegamos normal
        await goToNextSection();
    };

    const handleConfirmGoToExam = async () => {
        setShowExamWarning(false);
        await goToNextSection();
    };

    const handleCancelGoToExam = () => {
        setShowExamWarning(false);
    };
  ...
  return (
    <div ref={topRef} className="min-h-screen bg-blue-50 py-12">
      <div className="max-w-4xl mx-auto p-6 bg-white rounded-xl shadow-md">
        ...
        {currentSection?.content && (
          <div className="my-4">
            {currentSection.content.type === "VIDEO" &&
            ...
            {currentSection.content.type === "DOCUMENT" &&
              ...
              (() => {
                ...
                // 2) Resto de tipos
                const isDocx =
                  embedUrl?.toLowerCase().endsWith(".docx") ?? false;
            ...
            {currentSection.content.type === "IMAGE" &&
              currentSection.content.url && (
                <img
              ...
              <div className="space-y-6">
                ...
                {(currentSection.content.timeLimit ?? 0) > 0 && !examResult && (
                  <div className="fixed right-4 top-24 z-40">
                    <div
                      className={`rounded-xl border bg-white/90 px-4 py-2 shadow-lg backdrop-blur
                      ${
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-sm text-gray-600">
                      <span className="inline-flex items-center rounded-full border px-2.5 py-0.5">
                        ...
                        /{currentSection.content.questions?.length ?? 0}{" "}
                        respondidas
                      </span>
                    ...
                    <div className="flex items-center gap-3 rounded-lg border bg-green-50 px-3 py-2 text-green-800">
                      ...
                      <div className="text-sm">
                        <div className="font-semibold">
                          Enviado · Puntaje: {examResult.score} /{" "}
                ...
                <div className="space-y-5">
                  {currentSection.content.questions?.map((q, idx) => {
                    ...
                    const resByQ = examResult?.results?.find(
                      (r) => r.questionId === q.id,
                    );
                        ...
                        <div className="mb-3 flex items-center justify-between gap-3">
                          ...
                          {examResult ? (
                            <div className="flex items-center gap-2">
                              ...
                              {(() => {
                                const selectedCorrectCount = Array.from(
                                  selected,
                                ).filter((id) => correctSet.has(id)).length;
                                ...
                                return (
                                  <span className="inline-flex items-center rounded-full border border-gray-200 bg-white px-2.5 py-1 text-xs text-gray-600">
                                    {selectedCorrectCount}/{totalCorrectCount}{" "}
                                    correctas
                                  </span>
                                );
                              })()}
                          ...
                          {q.options.map((opt) => {
                            ...
                            if (!examResult) {
                              ...
                              if (selectedCorrect) {
                                ...
                                cls +=
                                  " border-dashed border-emerald-500 bg-white";
                              ...
                              cls +=
                                " hover:!bg-white hover:!shadow-sm hover:!border-inherit dark:hover:!bg-neutral-900";
                            }
                            ...
                            return (
                              ...
                              >
                                ...
                                {!examResult ? (
                                  <span
                                    className={`h-3 w-3 shrink-0 rounded-full ${isSelected ? "bg-blue-600" : "bg-gray-300"}`}
                                  />
                                ) : selectedCorrect ? (
            ...
            {currentSection.content.type === "SURVEY" && (
              <div className="space-y-6">
                {/* Encabezado */}
                <div className="flex items-start justify-between gap-4 rounded-xl border bg-white p-4 shadow-sm">
                  <div className="space-y-1">
                    <p className="text-lg font-semibold">
                      Encuesta de la sección
                    </p>
                    <div className="flex flex-wrap items-center gap-3 text-sm text-gray-600">
                      <span className="inline-flex items-center rounded-full border px-2.5 py-0.5">
                        {
                      ...
                      {surveyResult && (
                        <span className="inline-flex items-center rounded-full border px-2.5 py-0.5 text-green-700 bg-green-50 border-green-200">
                          Enviada
                        </span>
                      )}
                    </div>
                    ...
                    return (
                      ...
                      >
                        <div className="mb-3 flex items-center justify-between gap-3">
                          ...
                          {surveyResult && (
                            <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs border bg-green-50 text-green-700 border-green-200">
                              Respondida
                            </span>
                          )}
                        </div>
        ...
        <div className="flex justify-between mt-8">
          ...
          {currentSection.content.type === "EXAM" ? (
            ...
            surveyResult ? (
              ...
              <Button onClick={handleSubmitSurvey} disabled={submittingSurvey}>
                {submittingSurvey ? "Enviando..." : "Enviar encuesta"}
              </Button>
            )
          ) : (
            <Button onClick={handleNextSection}>
            ...
            {/* Modal de advertencia antes de entrar a la sección de EXAM */}
            {showExamWarning && nextSectionIsExam && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                    <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
                        <h2 className="text-xl font-semibold mb-2">
                            La próxima sección es un examen
                        </h2>
                        <p className="text-sm text-gray-600 mb-4">
                            Estás a punto de pasar a la sección de examen del curso.
                            <br />
                            Si todavía querés repasar el contenido anterior, podés cancelar y volver más tarde.
                        </p>

                        {typeof nextSection?.content?.timeLimit === "number" &&
                            (nextSection.content.timeLimit ?? 0) > 0 && (
                                <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
                                    Este examen tiene un límite de tiempo de{" "}
                                    <span className="font-semibold">
                    {nextSection.content.timeLimit} minuto
                                        {nextSection.content.timeLimit === 1 ? "" : "s"}
                  </span>
                                    .
                                </div>
                            )}
                        ...
                        <div className="flex justify-end gap-3 mt-4">
                            <Button variant="outline" onClick={handleCancelGoToExam}>
                                Seguir revisando
                            </Button>
                            <Button onClick={handleConfirmGoToExam}>
                                Ir al examen
                            </Button>
                        </div>
                    </div>
