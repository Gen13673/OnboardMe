...
> = {
  [RoleName.ADMIN]: [
    {
      key: "COURSE_USER_ELAPSED_DAYS",
      label: "Tiempo promedio de completitud de curso",
      icon: "⏳",
      color: "from-blue-500 to-indigo-600",
      chartType: "gauge",
      description:
        "Tiempo transcurrido por usuario desde inscripción a finalización (o hasta hoy) a nivel organizacional.",
      unit: " días",
    },
    {
      key: "USER_COURSE_COMPLETION",
      label: "Completitud de cursos por empleado",
      icon: "📚",
      color: "from-emerald-500 to-green-600",
      chartType: "bar",
      description:
        "Totales (completaron vs no) y desglose por empleado con cursos pendientes.",
      unit: "",
    },
    {
      key: "EXAM_RESULTS",
      label: "Resultados de examen",
      icon: "✅",
      color: "from-emerald-500 to-green-600",
      chartType: "bar",
      description: "Aprobados / desaprobados / pendientes por usuario del curso elegido.",
      unit: "",
    },
    {
      key: "SURVEY_FEEDBACK",
      label: "Feedback de cursos/buddies",
      icon: "🗳️",
      color: "from-indigo-500 to-violet-600",
      chartType: "bar",
      description: "Distribución de respuestas por pregunta y satisfacción con Buddy (por curso y general).",
      unit: "",
    },
  ],
  [RoleName.RRHH]: [
  ...
  [RoleName.BUDDY]: [
    {
      ...
      chartType: "gauge",
      description:
        "Tiempo transcurrido por usuario en tus asignados (desde inscripción a finalización o hasta hoy).",
      unit: " días",
    },
    {
      key: "USER_COURSE_COMPLETION",
      ...
      chartType: "bar",
      description:
        "Totales (completaron vs no) y desglose por empleado con cursos pendientes (solo tus asignados).",
      unit: "",
    },
    {
      key: "EXAM_RESULTS",
...
export default function MetricsPage() {
  ...
  const options = metricOptionsByRole[role]?.length
    ? metricOptionsByRole[role]
    : [];

  const [selected, setSelected] = useState<MetricTypeDTO>(
    "COURSE_USER_ELAPSED_DAYS",
  );
  const [data, setData] = useState<DataPointDTO[]>([]);
  const [isLoading] = useState(false);
  const [hoveredMetricKey, setHoveredMetricKey] =
    useState<MetricTypeDTO | null>(null);
  const metricHoverTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const currentDescription =
    currentOption?.description || "Visualización de datos.";
        ...
        <div className="flex items-center justify-center h-80">
          <div className="text-center">
            ...
            <p className="text-gray-400 text-sm">
              Intenta seleccionar otra métrica o verifica tus asignaciones.
            </p>
          </div>
        </div>
      );
          ...
          <ResponsiveContainer width="100%" height={400}>
            <PieChart>
              <Pie
                ...
                labelLine={false}
                label={({ name, percent }) =>
                  `${name} ${(percent * 100).toFixed(0)}${currentUnit}`
                }
              >
                {data.map((_, i) => (
                  <Cell
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-white/80 backdrop-blur rounded-xl shadow-lg border-0 p-8 text-center">
            ...
            <p className="text-gray-600 mt-2">
              Tu rol no tiene acceso al Dashboard de Métricas.
            </p>
          </div>
        </div>
      </main>
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-blue-50 via-blue-100 to-blue-200 p-6">
      <div className="max-w-7xl mx-auto space-y-3">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600 text-sm mt-1">
                  Analiza el rendimiento y progreso de la organización
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
              {options.map((option) => (
                <div
                  ...
                  className={`relative p-4 rounded-lg border cursor-pointer transition-all duration-300 ${selected === option.key
                ...
                >
                  <div className="flex items-center space-x-3">
                    <div
                      className={`h-10 w-10 rounded-lg flex items-center justify-center ${selected === option.key
        ...
        {/* Espaciador entre selector y métricas */}
        <div className="h-1 md:h-1"></div>

        {/* Bloques de las métricas */}

        {/* Contenido según métrica seleccionada */}
        {selected === "COURSE_USER_ELAPSED_DAYS" && (
          <CourseUserTime
            buddyId={role === RoleName.BUDDY ? user!.id : undefined}
          />
        )}
        ...
        {selected === "EXAM_RESULTS" && (
          <ExamResults buddyId={role === RoleName.BUDDY ? user!.id : undefined} />
        )}
        {selected === "SURVEY_FEEDBACK" && (
          <FeedbackSurvey buddyId={role === RoleName.BUDDY ? user!.id : undefined} />
        )}
      </div>
    </main>
  );
